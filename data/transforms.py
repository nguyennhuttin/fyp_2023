    ...
    def __call__(self, data):
        for t in self.transforms:
            try:
                ...
                data['audio'] = t(
                    data['audio'], sample_rate=data['sample_rate'])
        return data
...
class BPEtexts:
    ...
    def __call__(self, data):
        data['text'] = np.array(self.bpe.encode(
            data['text'], dropout_prob=self.dropout_prob))
        return data


class TextPreprocess:
    def __call__(self, data):
        data['text'] = data['text'].lower().strip().translate(
...
class ToGpu:
    ...
    def __call__(self, data):
        data = {k: [torch.from_numpy(np.array(item)).to(
            self.device) for item in v] for k, v in data.items()}
        return data


class AddLengths:
    def __call__(self, data):
        data['input_lengths'] = torch.tensor(
            [item.shape[-1] for item in data['audio']]).to(data['audio'][0].device)
        data['target_lengths'] = torch.tensor(
            [item.shape[0] for item in data['text']]).to(data['audio'][0].device)
        return data
        ...
        for k, v in data.items():
            if len(v[0].shape) < 2:
                ...
                padded_batch[k] = torch.nn.utils.rnn.pad_sequence(
                    items, batch_first=True)[..., 0]
            else:
                items = [item.permute(1, 0) for item in v]
                padded_batch[k] = torch.nn.utils.rnn.pad_sequence(
                    items, batch_first=True).permute(0, 2, 1)
        return padded_batch
...
class MelSpectrogram(torchaudio.transforms.MelSpectrogram):
    def forward(self, data):
        for i in range(len(data['audio'])):
            data['audio'][i] = super(
                MelSpectrogram, self).forward(data['audio'][i])
        return data
    ...
    def __init__(self, normalize=None, *args, **kwargs):
        ...
        if normalize == 'to05':
            ...
            self.normalize = lambda x: (
                x - torch.mean(x, dim=1, keepdim=True)) / (torch.std(x, dim=1, keepdim=True) + 1e-18)
        else:
            self.normalize = None

    def forward(self, data):
        for i in range(len(data['audio'])):
            melsec = super(NormalizedMelSpectrogram,
                           self).forward(data['audio'][i])
